 ...
 *     ≤5 条 + "还有 N 条"折叠。
 *
 *     约束流合并 A1（2026-07-12）范围边界（刻意，不是漏做）：房间模式下
 *     同一份台账数据现在还会额外喂给 `CollabBar.tsx` 顶栏摘要的合并展示
 *     流（`lib/collab-feed.ts::mergeCollabFeed`），本面板的「本次调整」区
 *     **不因此下线/隐藏**——A1 的删除范围明确点名的是 `ConstraintFeed.tsx`
 *     （独立约束流块）与移动端 `MobilePreferencesCard`（房间态台账卡，那
 *     是一个 100% 房间专属、除了台账区什么都不剩的组件，删整个函数不留
 *     技术债）；本组件是持续存在的"我的偏好画像"面板，单人/房间两态都要
 *     用（画像区/展开折叠/清空按钮等与台账无关的职责单人模式仍然需要），
 *     不是"只服务房间约束展示"的专用组件，直接类比删除会造成范围蔓延、
 *     误删单人模式功能。房间态下两处展示位服务不同场景、互不替代：本面板
 *     是"我的偏好档案"（个人视角），CollabBar 下拉是"房间协作动态一览"
 *     （全房视角，还带约束流条目/指名换店留痕，本面板不含这些）。
 *
