...
describe("store · 聊天气泡口播去重（卡片放全文，聊天放交接句）", () => {
  ...
  it("confirm onDone：信任带确认态保持——不清空 intent/thoughts/toolCalls/replans/lastRefinement/criticReport（用户拍板：确认预约不是规划动作，不该动信任带）", async () => {
    const fakeIntent = { understanding: "测试用户想吃火锅" } as unknown as ReturnType<
      typeof useChatStore.getState
    >["intent"];
    const fakeCriticReport = {
      violations: [{ code: "x", message: "y" }],
    } as unknown as ReturnType<typeof useChatStore.getState>["criticReport"];
    useChatStore.setState({
      itinerary: makeItinerary("和室友唱K，2.8小时"),
      narration: { text: "confirm 阶段的口播全文", stage: "confirm" },
      intent: fakeIntent,
      thoughts: [{ text: "在想吃什么", createdAtSeq: 1 } as never],
      toolCalls: [{ id: "t1", name: "search_restaurants" } as never],
      replans: [{ id: "r1" } as never],
      lastRefinement: { changedFields: ["distance"] } as never,
      criticReport: fakeCriticReport,
    });
    await useChatStore.getState().confirm();
    const s = useChatStore.getState();
    expect(s.intent).toBe(fakeIntent);
    expect(s.thoughts).toHaveLength(1);
    expect(s.toolCalls).toHaveLength(1);
    expect(s.replans).toHaveLength(1);
    expect(s.lastRefinement).not.toBeNull();
    expect(s.criticReport).toBe(fakeCriticReport);
    // 终态杂务仍要做：streaming 复位 + 交接句仍照常推
    expect(s.streaming).toBe(false);
    expect(s.streamPhase).toBe("idle");
    expect(s.messages.at(-1)?.text).toContain("都订好了");
  });
