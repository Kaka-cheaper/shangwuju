...
_STRONG_FEEDBACK_KEYWORDS: tuple[str, ...] = (
    ...
    #  约束，就剪；本词目逐语境推演后收录）】
    #  ① 问句形「大家有没有海鲜过敏的？」「这家有过敏原标注吗？」——在收集
    #    信息不是提约束 → 由 route_turn 的 B2 问句尾护栏（吗/呢/？/?）在拍板
    #    前挡掉，不落到本词表（互动核查见 test_allergy_question_guard_interplay）；
    #  ② 否定形「不过敏/没过敏/不会过敏」——不携带约束语义 → 收进
    #    _STRONG_SCAN_NOISE 剔噪（与「附近」同一机制，不发明第二套词法）；
    #    半句否定半句肯定（"对海鲜不过敏，但花生过敏"）剔噪后真诉求仍命中；
    ...
    #    安全约束优先于多数偏好）。代价不对称 + ①②护栏在位，按安全级硬约束
    #    收录。mock_data 全量 grep 无含「过敏」的实体名，无实体名碰撞面。
    "过敏",
)
...
#
# 任务 2 追加（同一机制，不另起第二套否定词法）：「过敏」的否定形——"不过敏/
# 没过敏/不会过敏"整体不携带约束语义，剔除后不影响同句里真正的过敏诉求
# （"对海鲜不过敏，但花生过敏"剩下的"花生过敏"仍命中）。长词在前，避免
# 先剔短词把长形拆残。
_STRONG_SCAN_NOISE: tuple[str, ...] = ("附近", "不会过敏", "不过敏", "没过敏")
