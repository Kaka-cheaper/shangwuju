...
新代码请直接 `from agent.node_decider import decide_nodes`。

参考 spec itinerary-edge-model-refactor R7 / Task 9：
    `segment_decider.py` 重命名为 `node_decider.py`，原文件保留
    `from .node_decider import *` 兼容 alias，避免外部 import 损坏。
"""

from .node_decider import *  # noqa: F401, F403
from .node_decider import (  # 显式 re-export（让 IDE 看得见）
    ALWAYS_INCLUDED,
    FULL_MID_NODES,
    FULL_SEGMENTS,
    KIND_DINING,
