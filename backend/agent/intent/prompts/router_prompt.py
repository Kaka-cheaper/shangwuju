...
这次输入是对现有方案的延续。请分清它是下面哪一种（这会决定下游怎么处理，务必分清）：
- **嫌方案、想改**（「太赶了 / 太满 / 想轻松点 / 这版不行 / 换个活动 / 第二个不喜欢」）
  → 判 **ambiguous**（下游当反馈重规划）
- **提了个新约束、但没明说要改方案**（「我妈膝盖不好」「我不太能吃辣」「带着老人腿脚不便」）
  → 判 **emotional**（下游会主动问要不要照此调整，而不是闷头重规划）
- **针对方案提问**（「那家贵不贵」「这个公园远吗」「要等位吗」）
  → 判 **ambiguous**（下游会查数据回答）
- **明确发起全新场景**（「换成和朋友打球」「周末带爸妈吃饭」「改成看电影」）且能抽出完整意图
  → 判 **planning**
- 拿不准 → 判 ambiguous，不要轻易判 planning
