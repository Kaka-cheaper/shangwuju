...
【时间钟点保形规则（I3 显式保真 · 关键）】
- 用户明说**具体钟点**（「晚上七点」「19 点出发」「七点半出门」）→ start_time
  **必须保留钟点形式**（如 "19:00" 或 ISO-like "2026-07-12T19:30"），**禁止**
  归一成时段 token（today_evening）——钟点一旦丢形，下游的出发时刻披露机制
  永远无法与用户说的钟点对账，明说的时刻等于被静默吞掉。
- 只说**时段**（「下午」「晚上」「周末下午」）→ 照旧用时段 token
  （today_afternoon / weekend_afternoon 等），不要编造具体钟点。

【终点/门禁表达自报规则（诚实信号）】
- 「十点前得回家」「最晚 9 点得回去」这类**终点门禁**当前 schema 没有专门
  字段——照旧把它折算进 duration_hours 区间，**同时必须**把 "return_by" 写进
  ambiguous_fields，如实自报"听到了门禁、但没有字段能精确承接"——下游会把
  预计到家时刻披露给用户自己核对（与 budget_per_person 定性自报同一条纪律）。
