...
7. null-on-removal 撤回信道（forge-intent-loss 收敛，2026-07-12，RFC 7386
   "键缺失=不动、值显式 null=删除"语义的 prompt 化落地）：教 LLM 对
   preferred_poi_types / extra_services / budget_per_person 这类可选字段，
   撤回诉求时必须输出显式空值（[]/null），不能靠省略键表达撤回——省略键在
   refiner.py 的通用键缺失继承守卫（`_inherit_missing_keys`）里等价"没提，
   继承原值"，如果 LLM 把撤回也写成省略键，守卫会把撤回的旧值继承回来，
   撤回就白撤了。explicit_dining_requested 三态规则是这条纪律最早的字段级
   示范（四条不变式批 I3），本次推广到其余可选字段（见【显式撤回一个诉求 vs
   这次没提】节 + few-shot 10/11/12）。否定词表（不要/不吃/算了/换成…）是
   代码侧的兜底矛盾检测，不是本条 prompt 规则的替代——prompt 教得越准，
   兜底越少被触发。
...
【显式撤回一个诉求 vs 这次没提——两者写法必须不同（关键 · 防止诉求悄悄消失）】
下面这些字段如果**这次反馈完全没提及**，正确做法是**原样复制原值**（第 4 条已经
要求了）；但如果用户在反馈里**明确表示不要这个诉求了**（撤回/改口，不是没提），
你必须**显式输出该字段的空值**（列表填 `[]`、数字/字符串填 `null`），**不能靠
"忘了填"来表达撤回**——省略字段在这套 schema 里等价于"没提，原样保留"，不是
"撤回"，你在输出层面一定要让这两种情况长得不一样：
- `preferred_poi_types` / `extra_services`（列表）：反馈明确说"不吃 X 了/不去
  Y 了/算了不用 Z"这类撤回，输出该字段为**显式 `[]`**（不是省略键）；反馈只字
  未提这个诉求 → 照抄原值。
- `budget_per_person`（数字）：反馈明确说"不设预算了/别卡预算了/随便花多少都
  行" → 输出**显式 `null`**（不是省略键，也不是编造新数字）；反馈没提预算 →
  照抄原值。
- `explicit_dining_requested` 同款三态规则见下方【反馈意图分类】——这是本条
  规则最早的示范字段，其余可选字段照此类推。
- **对照（防矫枉过正）**：反馈只改了别的字段、这个字段确实没提到 → 照抄原值
  即可，**不要**因为学了"撤回要显式清空"就把"没提"也当成撤回处理。
...
- 预算说了具体数字（"预算 200" / "人均 150" / "200 块钱以内"）
                                → budget_per_person 设为该数字（float）；只说"贵/便宜"没给
                                  ...
                                  决策 3：系统不编造用户没说的话）
- "预算不设限了" / "别卡预算了" / "随便花多少都行"（明确撤回预算限制，不是没说）
                                → budget_per_person 输出**显式 null**（不是省略键、也不是
                                  编造新数字——见上方【显式撤回一个诉求 vs 这次没提】）
...
- "不吃 X 了" / "不去 Y 了"（撤回此前明说过的 preferred_poi_types 品类，不是没提）
                                → 该品类从 `preferred_poi_types` 里去掉，若这是列表里
                                  唯一一项 → 输出**显式 []**（不是省略键——见上方
                                  【显式撤回一个诉求 vs 这次没提】）
...
REFINER_FEW_SHOTS: list[tuple[str, str]] = [
    ...
    # 10. null-on-removal · 列表字段显式撤回：上轮已明说"烧烤"（preferred_poi_types
    #     非空），这轮反馈明确撤回该品类诉求 → 输出显式 []（不是省略键）。
    #     省略键在这套 schema 里等价"没提"，会被继承守卫当"忘写"补回旧值，
    #     撤回就白撤了——所以撤回必须是显式空值，这是本 few-shot 要示范的核心。
    (
        '原 intent={"start_time":"today_afternoon","duration_hours":[2,3],'
        '"distance_max_km":5,"companions":[{"role":"自己","count":1}],'
        '"physical_constraints":[],"dietary_constraints":[],'
        '"experience_tags":[],"social_context":"独处放空",'
        '"raw_input":"想吃个烧烤","parse_confidence":0.85,"ambiguous_fields":[],'
        '"start_weekday":null,"capacity_requirement":null,'
        '"extra_services":[],"preferred_poi_types":["烧烤"]}'
        ' | feedback="不吃烧烤了，随便逛逛就行"',
        '{"refined_intent":{"start_time":"today_afternoon","start_weekday":null,'
        '"duration_hours":[2,3],"distance_max_km":5,'
        ...
        '"explicit_dining_requested":null,'
        '"raw_input":"想吃个烧烤","parse_confidence":0.85,"ambiguous_fields":[],'
        '"understanding":"用户说不吃烧烤了随便逛逛，我理解成先撤掉这个品类诉求"},'
        '"changed_fields":["去品类：烧烤"],'
        '"refiner_note":"明白，先不定具体吃什么，随便逛逛看。"}',
    ),
    # 11. null-on-removal · 预算显式撤回：上轮已明说预算 200（budget_per_person
    #     非空），这轮反馈明确撤回预算限制 → 输出显式 null（不是省略键、也不是
    #     编造新数字）。同上，省略键会被继承守卫当"忘写"补回 200，达不到用户
    #     "不设限"的真实意图。
    (
        '原 intent={"start_time":"today_evening","duration_hours":[3,4],'
        '"distance_max_km":5,"companions":[{"role":"朋友","count":2}],'
        '"physical_constraints":[],"dietary_constraints":[],'
        '"experience_tags":["热闹"],"social_context":"朋友热闹",'
        '"raw_input":"晚上和朋友聚聚，预算给到200","parse_confidence":0.86,'
        '"ambiguous_fields":[],"start_weekday":null,"capacity_requirement":null,'
        '"extra_services":[],"preferred_poi_types":[],"budget_per_person":200}'
        ' | feedback="预算不设限了，好吃的就行"',
        '{"refined_intent":{"start_time":"today_evening","start_weekday":null,'
        '"duration_hours":[3,4],"distance_max_km":5,'
        '"companions":[{"role":"朋友","age":null,"count":2,'
        '"is_birthday":false,"is_special_role":false}],'
        '"physical_constraints":[],"dietary_constraints":[],'
        '"experience_tags":["热闹"],"social_context":"朋友热闹",'
        '"capacity_requirement":null,"extra_services":[],"preferred_poi_types":[],'
        '"explicit_dining_requested":null,"budget_per_person":null,'
        '"raw_input":"晚上和朋友聚聚，预算给到200","parse_confidence":0.86,'
        '"ambiguous_fields":[],'
        '"understanding":"用户说预算不设限了，我理解成先撤掉这条预算限制"},'
        '"changed_fields":["预算：200 元/人 → 不限"],'
        '"refiner_note":"好，这次不卡预算了，优先给你挑好吃的。"}',
    ),
    # 12. 对照·「没提≠撤回」：反馈只改了距离，preferred_poi_types（烧烤）与
    #     budget_per_person（200）这次只字未提 → 照抄原值，不能因为刚学了
    #     "撤回要显式清空"就把"没提"也误判成撤回、矫枉过正地清空这两个字段。
    (
        '原 intent={"start_time":"today_afternoon","duration_hours":[2,3],'
        '"distance_max_km":8,"companions":[{"role":"自己","count":1}],'
        '"physical_constraints":[],"dietary_constraints":[],'
        '"experience_tags":[],"social_context":"独处放空",'
        '"raw_input":"想吃个烧烤，预算给到200","parse_confidence":0.85,'
        '"ambiguous_fields":[],"start_weekday":null,"capacity_requirement":null,'
        '"extra_services":[],"preferred_poi_types":["烧烤"],"budget_per_person":200}'
        ' | feedback="太远了，近一点"',
        '{"refined_intent":{"start_time":"today_afternoon","start_weekday":null,'
        '"duration_hours":[2,3],"distance_max_km":4.8,'
        '"companions":[{"role":"自己","age":null,"count":1,'
        '"is_birthday":false,"is_special_role":false}],'
        '"physical_constraints":[],"dietary_constraints":[],'
        '"experience_tags":[],"social_context":"独处放空",'
        '"capacity_requirement":null,"extra_services":[],"preferred_poi_types":["烧烤"],'
        '"explicit_dining_requested":null,"budget_per_person":200,'
        '"raw_input":"想吃个烧烤，预算给到200","parse_confidence":0.85,'
        '"ambiguous_fields":[],'
        '"understanding":"用户说太远了要近一点，我理解成先把范围收紧"},'
        '"changed_fields":["距离上限：8km → 4.8km"],'
        '"refiner_note":"已把范围缩到 4.8 公里以内，烧烤和预算都还按原来的来。"}',
    ),
]
