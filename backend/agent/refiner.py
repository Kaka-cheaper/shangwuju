...
- _rule_fallback 加 _KEYWORDS_SESSION_TOO_LONG（"太久" "太长" "盯不住" "无聊"
  "扛不住" "腻了"）；命中后**不动 duration_hours / distance_max_km**，而产出
  pace_profile.single_session_max_min 缩 30%（如原 90 → 65）。这让 refiner
  能在用户说"这段太长了"时只调"节奏"维度，不破坏总时长。
...
# "这段太长 / 太久 / 盯不住 / 腻了" 类反馈表示**单段节奏太长**，不是总时长太长——
# 命中后**不动 duration_hours / distance_max_km**，而是缩 pace_profile.single_session_max_min
# 30%（让 critic + planner 看到更紧的单段上限，自然把单段缩短）。
_KEYWORDS_SESSION_TOO_LONG = (
    "太久", "太长", "盯不住", "无聊", "扛不住", "腻了",
)

# 单段缩比（30%）。原 single_session_max_min × 0.7，向下取整再回填。
_SESSION_SHRINK_RATIO = 0.7

# 没有任何 prior 时的默认 single_session_max_min（分钟），用于触发"太久"反馈但
# 无 pace_profile 也无 default 时的兜底。约等于成人单点平均时长。
_DEFAULT_SESSION_MAX_MIN = 90
...
def _rule_fallback(
    ...
    # spec planning-quality-deep-review R8（Task 7）：
    # SESSION_TOO_LONG 反馈 → 缩 pace_profile.single_session_max_min 30%
    # **不动** duration_hours / distance_max_km（这是 task 描述里的硬约束）
    if any(k in feedback for k in _KEYWORDS_SESSION_TOO_LONG):
        old_pace: PaceProfile | None = original.pace_profile
        old_sess = (
            old_pace.single_session_max_min
            if old_pace is not None and old_pace.single_session_max_min is not None
            else _DEFAULT_SESSION_MAX_MIN
        )
        new_sess = max(15, round(old_sess * _SESSION_SHRINK_RATIO))  # 不低于 15min 兜底
        if new_sess != old_sess or old_pace is None:
            base_dump = (
                old_pace.model_dump() if old_pace is not None else {}
            )
            base_dump["single_session_max_min"] = new_sess
            updates["pace_profile"] = PaceProfile.model_validate(base_dump)
            changed.append(
