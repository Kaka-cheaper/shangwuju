...
{{
  ...
  "rationale": "为什么这么排（不写具体钟点——最终时刻由系统折叠计算，写了会与时间轴不符）",
...
【饭：条件性主角，非硬编码必到】
- intent.explicit_dining_requested=true（用户明说要吃饭）→ 必须排一顿（茶点诉求排下午茶/甜品即可）；=false（明说不要排饭）→ 不排任何餐厅节点，压过下面所有推断。
- 没提（null）时按推断：仅当 social_context ∈ {{商务接待, 纪念日仪式感}}，或行程跨某饭点窗且有 dietary_constraints 信号时，才把饭当主角：排在对应饭点、给足 typical_dining_min。
