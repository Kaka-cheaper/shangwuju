...
【与既有 `IntentExtraction.pace_profile` / `PaceProfile` 的关系（刻意不复用）】

`schemas/persona.PaceProfile`（spec planning-quality-deep-review R1+R8 引入）是
一套**数值型**节奏画像（`single_session_max_min` / `total_active_min` /
`break_every_min` / `preferred_dwell_min`），来源是 LLM 意图解析注入 prompt 的
prior + refiner "太久/盯不住" 反馈缩水回写——服务的是 **LLM 主路径的 prompt
注入与迭代反馈**，目前未被任何 planner/critic 代码实际消费（`grep` 确认只有
...
推（与驱动 age cap 同一批信号）"——明确要求从既有字段**重新推导**，不是读
`pace_profile`。两者目前正交共存：`pace_profile` 服务 prompt/refiner 反馈环，
本模块的 `pace()` 服务 D-2/D-4 的 TOPTW 区间填充与 slack 分配。是否应该让二者
互相影响（如 `pace_profile.single_session_max_min` 存在时收紧/放宽某档
`slack_fraction`）超出 D-3 范围，留给后续步骤按需评估——本模块不读
`pace_profile` 字段，避免在没有实测前先验地假设两套信号该如何合流。
...
- `IntentExtraction.pace_profile` 的读取/合流（见判断点，刻意留白）。
"""
