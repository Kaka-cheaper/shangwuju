...
【位置保持修复（2026-07-10）：换菜不该让保留节点跟着挪位置】

真机截图实证的 bug：两活动方案换 1 号位节点，正确结果应是"1 号位变成替补、
2 号位保留节点原地不动"，实际观察到的却是"1 号位变成了原 2 号位的场所、
2 号位变成了替补"——成员级换对了（该出的出、该进的进、没有第三方被误伤），
但**位置/时序级**错位，用户读作"换错了卡"，是真实的信任伤害（路演语境下
尤其致命）。

根因：`route_builder.repair_route` 修复"缺口"时调用 `schedule_route(kept,
...)`——`route_scheduler` 判断点 1 讲得很清楚，`schedule_route` 是**全排列
枚举**求最优序，这个语义对"首次构造路线"（顺序本就没定过）是对的，但换菜
时 `kept` 已经是**用户看到过、认可过的顺序**，全排列允许把它整体打乱重排，
"钉住其余节点不动"这句 min-conflicts 式承诺（本模块 docstring 开篇「这是
什么问题」节）在**成员**维度成立、在**位置**维度被击穿——`try_insert` 同一
病灶（其 docstring 原话"不保留 existing 原有顺序，允许整体重新排列"）。
这是"算子被复用到新场景后，原场景对'顺序未定'的隐含前提没有跟着一起搬"的
典型：`schedule_route`/`try_insert` 最初只服务"从空构造一条路线"，那里顺序
确实未定、全排列找最优无害；`repair_route` 被 ADR-0013 F-1 复用到"缺口修复"
后，这个"顺序未定"的前提对"腾出的这一个坑"成立，对"其余保留坑"不成立，
但实现没有区分这两者。

修法（`route_builder.repair_route` 新增 `preserve_position` opt-in，本模块
是唯一传入方——ILS critic 修复闭环 `ils_planner.plan_hybrid` 的整体重排是
合理且必须保留的既有语义，不能改，见 `repair_route` 自身 docstring
「`preserve_position`」节的完整设计说明）：
1. `resolve_node_swap` 算出目标节点在"去掉它之后的保留列表"里的原序位
   （`preserve_position`，数非 home 节点、按方案实际顺序，见函数体内该
   变量旁的注释）；
2. 替补候选按 `kept[:序位] + [候选] + kept[序位:]` 这个**固定顺序**用新增的
   `route_scheduler.try_order_fixed`（`schedule_route` 内部本就用来逐序评估
   的原语 `_try_order` 的薄公开封装，不新写第二份定序调度逻辑）求可行排程，
   不枚举、不重排；
3. 固定顺序对候选池里**所有**候选都排不开时，`repair_route` 诚实退让回
   现行为（`schedule_route`/`try_insert` 的全排列路径），并通过
   `reorder_flag_out` 告知调用方——本模块据此产出 `AdvisoryCode.
   SWAP_REORDERED`（`_swap_reordered_advisory`/`_reordered_advisories`），
   绝不静默地让用户以为"只换了这一格"其实其余节点先后顺序也变了。

候选评分逻辑不变——"固定顺序内可行候选按 `route_score` 边际分选最优"与
改动前"全排列可行候选按边际分选最优"是同一个选优标准，只是可行性判据从
"存在某个可行序"收窄成"这一个序可行"，两处 `_best_reordering_candidate`/
`_best_fixed_position_candidate`（`route_builder.py`）互为对称实现。
...
【分界修缮批 任务 1（2026-07-04）：换菜产物复跑 critic HARD 判据】

这是什么问题：generate-then-validate（LLM-Modulo/主管线既定范式）里 validator
覆盖不全的旁路问题。主管线的方案定稿要过 critic gate（`validate` 注册表全部
HARD 判据），而换菜是 HTTP/WS 旁路，此前引擎自带的 hard 防线只有候选池恒定
过滤（上节，只盖 dietary/physical tag 子集）——`check_capacity`（桌型）、
`check_demo_restaurant_full`（真实预约槽，调度器刻意保留满座槽点、满座本该由
critic 抓，见 `activity_pool` 槽单求交注释）、`check_social_context` BLOCKING
等其余 HARD 判据在换菜路径**无人执行**：6 人局可换进 2 人桌的店、唯一槽已满
的店可被换入，确认时 `replay_confirm_actions` 对失败订单静默 continue，
"已确认"叙事下订位没成。

修法（普查候选 a）：对每个 tier 候选产出的 `new_itinerary` 复跑 critic HARD
判据——直接用 `critic.validate.validate` 注册表 + 手工构造的 `CriticContext`
（`pois`/`restaurants` 用**调用方传入的候选池**做真值源，与引擎自身的反查
真值源一致；不走 `validate_itinerary` 的全量 mock 重载，合成候选/房间现场
重查池在两处看到的是同一个世界），**不抄第二份判据逻辑**。命中即视为该候选
不可行（加入本次调用的局部拒绝集、从当前池中剔除后重试），天然融进既有
tier 降级循环，不建新循环。

**基线容差（delta 判定）**：只拦"换菜新引入"的 HARD 违规——先对传入的原
`itinerary` 跑一次同款判据做基线，候选产物的 HARD 键集合减去基线，非空才
拒绝。身份键取 `(违规码, 该违规 field_path 点名节点的 target_id 或 None)`
（`_hard_violation_keys`）：换菜后节点顺序/时刻整体微移会让 message 措辞与
`nodes[i]` 下标漂移，按"码 + 实体"比对才是"同一处问题"的稳定身份。不做
绝对判定的原因：原方案可能带着历史合法但现约束下违规的节点（约束后收紧），
绝对判定会让这类方案彻底不能换菜——那不是本旁路该管的（定稿审计/重规划
才是），本步只保证"换菜不把方案改得更糟"。房间路径共用本引擎自动受益。

**归因分桶（收口深审精化，2026-07-05）**：introduced delta 按身份键落点分
两桶——(a) **候选自身归因**（target_id == 换入候选，或 None 的全程性/结构性
违规）→ 维持一票否决：记拒绝集、继续降级序列（v1 语义不变）；(b) **保留节点
殃及**（身份键落在保留节点上）→ 不拒绝该格。v1 对 (b) 也一票否决曾造成演示
可达路径的大面积误伤（两条房间测试实锤 + 合成探针复现同一根因链）：替补时长
与原节点不同 → 重排时刻整体平移 → 被保留的餐厅被调度器吸附到 available=False
的槽点（调度器**刻意**不按 available 过滤满座槽，见 `activity_pool` 槽单求交
注释——满座本该由 critic 抓），真实 mock 世界里"点踩 POI 且方案含餐厅"几乎
必踩中 → 每个候选都被记上保留餐厅的 RESTAURANT_FULL_UNRESOLVED → 候选全灭。
这是 repair-then-revalidate 的经典归因问题：该违规是"这次修复的副作用"而非
"该候选不合格"，殃及不该记在候选头上。

(b) 桶走 **clean-first, honest-fallback 两段式**：扫描时仍跳过殃及候选、继续
找"零殃及"的干净者（干净者存在就必须选干净者）；全部 tier 扫完仍无干净候选
时，回退交付**第一个仅殃及保留节点**的候选（tier 序即偏好序，只存第一个），
附 `SWAP_KEPT_TIME_SHIFTED` 诚实告知（点名受累节点 + 新排定时刻，走既有
advisory 尾拼通道，绝不静默）。措辞不超诺：下单期的槽位交叉校验
（`execute_finalize._reservation_time`）只做"note 时刻是真实可订槽才生效、
否则退用 start_time"，没有"自动重新对齐预约"的能力，所以只提醒用户确认。
更深的正解——**保留节点时刻的槽感知重排**（把"保留餐厅必须落在
available=True 的槽"作为调度约束进重排，而不是排完再验）——记档 E 系列演进
项，不在本批做：那要动 route_scheduler/activity_pool 的窗构造语义，而"保留
满座槽点"是 grounding 的刻意设计，牵动修复闭环"满座→改期"的旗舰链。
...
from ..critic._rules.types import Severity, Violation
from ..critic.context import CriticContext
from ..critic.exit_audit import _PROVENANCE_CLAUSE, _render_message, _unmet_soft_tags
from ..critic.validate import validate as validate_plan
...
# ============================================================
# 换菜产物复跑 critic HARD 判据（分界修缮批 任务 1；见模块 docstring）
# ============================================================

_NODE_FIELD_PATH_RE = re.compile(r"^nodes\[(\d+)\]")


def _swap_recheck_ctx(
    intent: IntentExtraction,
    user_profile: UserProfile,
    pois: Sequence[Poi],
    restaurants: Sequence[Restaurant],
) -> CriticContext:
    """复跑判据的 CriticContext——真值源用**调用方传入的候选池**（与引擎自身
    `_entity_by_id` 反查同一个世界），不走 `validate_itinerary` 的全量 mock
    重载：单人路径两者恰好相同（graph_adjust 传全量目录），房间路径/测试的
    现场重查池、合成实体只有这样才会被判据看见。`tool_results=None` → 反幻觉
    检查天然跳过（换菜候选全部来自池内反查，无幻觉面）。"""
...
def _hard_violation_keys(violations: Sequence[Violation], itinerary: Itinerary) -> set[tuple]:
    """HARD violation → `(违规码, 该违规点名节点的 target_id 或 None)` 身份键
    集合，供"基线 vs 换菜产物"delta 比对（见模块 docstring「基线容差」）。

    为什么不用 message / field_path 原文做身份：换菜后节点顺序与时刻整体微移
    会让 message 里的站名序号/时刻、`nodes[i]` 下标一起漂——按"码 + 实体"
    比对才是"同一处问题"的稳定身份。field_path 不含节点下标的违规（如
    DURATION_OUT_OF_RANGE 的全程性判定）target 记 None，退化为码级比对。
    """
    keys: set[tuple] = set()
    for v in violations:
        if v.severity != Severity.HARD:
            continue
        target_id = None
        m = _NODE_FIELD_PATH_RE.match(v.field_path or "")
        if m:
            idx = int(m.group(1))
            if 0 <= idx < len(itinerary.nodes):
                target_id = itinerary.nodes[idx].target_id
        keys.add((v.code, target_id))
    return keys


def _kept_shift_advisories(new_itinerary: Itinerary, kept_faults: set[tuple]) -> list[Advisory]:
    """honest-fallback 的殃及告知（模块 docstring「归因分桶」节）——每个受累
    保留节点一条，点名节点 + 新排定时刻。措辞不超诺：下单期的槽位交叉校验
    （`execute_finalize._reservation_time`）只做"note 时刻是真实可订槽才生效、
    否则退用 start_time"，没有"自动重新对齐预约"的能力——所以只提醒确认，
    不宣称已替用户处理。同一实体多条违规码只告知一次（消息以节点为单位）。"""
    fault_ids = {target for _, target in kept_faults if target}
    out: list[Advisory] = []
    for node in new_itinerary.nodes:
        if node.target_kind == "home" or node.target_id not in fault_ids:
            continue
        fault_ids.discard(node.target_id)  # 重复 id 防御：同一实体只告知一次
        label = node.title or node.target_id
        if node.target_kind == "restaurant":
            message = (
                f"提醒一下：换掉这一站后，「{label}」的到店时间挪到了 {node.start_time}，"
                "原本可订的时段可能对不上，确认下单前先核对一下这家的位子。"
            )
        else:
            message = (
                f"提醒一下：换掉这一站后，「{label}」挪到了 {node.start_time}，"
                "可能和它的开放时段对不上，出发前再确认一下。"
            )
        out.append(Advisory(code=AdvisoryCode.SWAP_KEPT_TIME_SHIFTED, message=message))
    return out
...
def _attempt_pool(
    ...
    *,
    ...
    `preserve_position`/`reorder_flag_out`：位置保持修复（见模块 docstring
    「位置保持」节与 `route_builder.repair_route` docstring「preserve_
    position」节）——本调用把 `preserve_position` 原样转交给 `repair_route`
    的同名 opt-in 形参，`reorder_flag_out` 原样转交（若发生"固定序位排不开、
    退回全排列"的降级，`repair_route` 会写 `reorder_flag_out["reordered"]
    = True`，调用方 `resolve_node_swap` 据此产出 `SWAP_REORDERED`）。
    """
    if not entities:
        return None
    ...
    return repair_route(
        ...
        reorder_flag_out=reorder_flag_out,
    )
...
def _swap_reordered_advisory() -> Advisory:
    """位置保持修复的诚实退让告知（`AdvisoryCode.SWAP_REORDERED`，见 schemas/
    advisory.py 该码 docstring）——只在 `repair_route` 确实从"固定序位插回"
    降级到"整体重排选最优"时才产出（见 `_reordered_advisories`）。"""
    return Advisory(
        code=AdvisoryCode.SWAP_REORDERED,
        message="为了把新的这一站排进去，方案里几站的先后顺序跟着调整了一下，不只是换了这一格，麻烦确认一下新顺序。",
    )


def _reordered_advisories(reorder_flag: dict) -> list[Advisory]:
    """`reorder_flag_out`（`repair_route`/`_attempt_pool` 的输出容器）→
    advisory 列表的转换 helper——`{"reordered": True}` 才产出一条，其余
    （空 dict / `{"reordered": False}`）不产出，与其它"无内容不加字段"
    advisory helper 同一纪律。抽成独立函数是因为两处调用点（clean 成功路径
    / honest-fallback 路径）都要做同一件事，不重复内联判断逻辑。"""
    return [_swap_reordered_advisory()] if reorder_flag.get("reordered") else []
...
def _finish_swap_success(
    *,
    ...
    swapped_to: str,
    tier_index: int,
    extra_advisories: Sequence[Advisory] = (),
) -> SwapResult:
    """成功交付的共用收尾——干净路径与 honest-fallback 路径同一实现（收口
    精化时从 `resolve_node_swap` 成功分支原样抽出，避免两条路径复制降级
    advisory / 换后单点审计 / SwapResult 构造这三件事）。`extra_advisories`
    两条路径都可能传：fallback 路径传 `_kept_shift_advisories`（殃及告知）+
    `_reordered_advisories`；clean 路径（位置保持修复新增）只传
    `_reordered_advisories`（`_kept_shift_advisories` 不适用——clean 路径
    定义就是"零殃及"）。均追加在既有 advisory 之后走同一尾拼通道。"""
    new_entity = _entity_by_id(kind, swapped_to, pois, restaurants)
    advisories: list[Advisory] = []
    if tier_index == 3 and adjustment is not None:
        advisories.append(_swap_degraded_advisory(new_entity.name if new_entity else swapped_to))
    ...
    new_node_match = next(
        (
            ...
            if node.target_kind == kind and node.target_id == swapped_to
        ),
        None,
    )
    ...
    advisories.extend(extra_advisories)
    return SwapResult(
        success=True,
...
def resolve_node_swap(
    ...
    *,
    ...
    # 位置保持修复（2026-07-10，见 route_builder.repair_route docstring
    # 「preserve_position」节）：目标节点在"去掉它之后的保留列表"里应该被
    # 插回的序位——数 `full_scheduled`（`itinerary.nodes` 的非 home 节点，
    # 按方案里的实际先后顺序）里排在目标节点之前、且没被这次换菜黑名单的
    # 节点数。`full_scheduled` 恒含目标节点且只含一次（`_find_target_node`
    # 前置条件 1 已保证目标在 itinerary.nodes 里唯一存在），故这里直接数
    # 下标不会歧义。这个序位在整个 resolve_node_swap 调用期间是常量——
    # 不随降级序列/ledger 分组变化，只算一次。
    preserve_position = next(
        i for i, sv in enumerate(full_scheduled) if sv.visit.target_id == target_node_id
    )
    ...
    # 分界修缮批 任务 1：复跑判据的上下文 + 原方案基线（见模块 docstring
    # 「换菜产物复跑 critic HARD 判据」节）。基线只算一次；delta 判定见循环内。
    critic_ctx = _swap_recheck_ctx(intent, user_profile, pois, restaurants)
    baseline_hard = _hard_violation_keys(validate_plan(itinerary, critic_ctx), itinerary)
    excluded_ids: set[str] = set()
    # 收口精化（模块 docstring「归因分桶」节）：仅殃及保留节点的候选不拒绝、
    # 暂缓为回退项——只存第一个（tier 序即偏好序）。元组：(new_itinerary,
    # swapped_to, tier_index, kept_faults, reordered)。`reordered`（位置保持
    # 修复新增）：这个回退候选是否也触发了固定序位→全排列的降级，见下方
    # `reorder_flag` 注释。
    fallback: Optional[tuple[Itinerary, str, int, set[tuple], bool]] = None
    ...
    for tier_index, tier_entities in enumerate(tiers, start=1):
        for group in _ledger_priority_groups(kind, tier_entities, target_entity, ledger_slice):
            pool = [e for e in group if e.id not in excluded_ids]
            while pool:
                # 位置保持修复：每次尝试用新的输出容器——`repair_route` 只在
                # 发生降级时写入，不发生则保持空 dict，不会把上一次尝试（比如
                # 被 hard 判据一票否决、已经从 pool 移除的那个候选）的降级
                # 状态串到这一次判定上。
                reorder_flag: dict = {}
                ...
                # 分界修缮批 任务 1：换菜产物复跑 critic HARD 判据——只看
                # "换菜新引入"的违规（基线 delta），并按归因分桶（见模块
                # docstring）：候选自身归因 → 一票否决；仅殃及保留节点 →
                # 暂缓为回退项，先继续找零殃及的干净候选。两种情况都把该
                # 候选移出当前池再重试（否则 repair_route 会反复选中同一个）。
                introduced = (
                    _hard_violation_keys(validate_plan(new_itinerary, critic_ctx), new_itinerary)
                    - baseline_hard
                )
                if introduced:
                    self_faults = {
                        (code, target)
                        for code, target in introduced
                        if target == swapped_visit.target_id or target is None
                    }
                    kept_faults = introduced - self_faults
                    if self_faults:
                        logger.info(
                            "resolve_node_swap: 候选 %s:%s 复跑 critic 判据未过，自身归因 HARD 违规 %s，"
                            "一票否决并继续降级序列（tier %d）",
                            kind,
                            swapped_visit.target_id,
                            sorted(str(code) for code, _ in self_faults),
                            tier_index,
                        )
                    else:
                        if fallback is None:
                            fallback = (
                                ...
                                tier_index,
                                kept_faults,
                                bool(reorder_flag.get("reordered")),
                            )
                        logger.info(
                            "resolve_node_swap: 候选 %s:%s 仅殃及保留节点 %s（重排时刻平移），"
                            "暂缓为回退项，继续找零殃及候选（tier %d）",
                            kind,
                            swapped_visit.target_id,
                            sorted(str(t) for _, t in kept_faults),
                            tier_index,
                        )
                    excluded_ids.add(swapped_visit.target_id)
                    pool = [e for e in pool if e.id != swapped_visit.target_id]
                    continue

                return _finish_swap_success(
                    kind=kind,
                    intent=intent,
                    pois=pois,
                    ...
                    extra_advisories=_reordered_advisories(reorder_flag),
                )

    if fallback is not None:
        # honest-fallback：没有零殃及的干净候选，交付第一个仅殃及保留节点的
        # 候选 + 诚实告知（绝不静默，也不再像 v1 那样整体拒换误伤演示主路径）。
        fb_itinerary, fb_swapped_to, fb_tier, fb_kept_faults, fb_reordered = fallback
        logger.info(
            "resolve_node_swap: 无零殃及候选，回退交付 %s:%s（tier %d）并附殃及告知 %s",
            kind,
            fb_swapped_to,
            fb_tier,
            sorted(str(t) for _, t in fb_kept_faults),
        )
        return _finish_swap_success(
            kind=kind,
            intent=intent,
            ...
            restaurants=restaurants,
            adjustment=adjustment,
            new_itinerary=fb_itinerary,
            swapped_to=fb_swapped_to,
            tier_index=fb_tier,
            extra_advisories=[
                *_kept_shift_advisories(fb_itinerary, fb_kept_faults),
                *_reordered_advisories({"reordered": fb_reordered}),
            ],
        )
