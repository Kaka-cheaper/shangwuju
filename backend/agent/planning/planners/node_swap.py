...
def _finish_swap_success(
    *,
    ...
    只有 fallback 路径会传（`_kept_shift_advisories` 的殃及告知），追加在
    既有 advisory 之后走同一尾拼通道。"""
...
def resolve_node_swap(
    ...
    *,
    ...
    # swapped_to, tier_index, kept_faults)。
    fallback: Optional[tuple[Itinerary, str, int, set[tuple]]] = None
            ...
            while pool:
                ...
                if introduced:
                    ...
                    if self_faults:
                        ...
                        if fallback is None:
                            fallback = (new_itinerary, swapped_visit.target_id, tier_index, kept_faults)
                        logger.info(
    ...
    if fallback is not None:
        ...
        fb_itinerary, fb_swapped_to, fb_tier, fb_kept_faults = fallback
        logger.info(
        ...
        return _finish_swap_success(
            ...
            tier_index=fb_tier,
            extra_advisories=_kept_shift_advisories(fb_itinerary, fb_kept_faults),
        )
