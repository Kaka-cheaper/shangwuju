...
不负责：
- 谁在何时调用这两个类去接线（E-2-c 已双侧接通：单人 `graph/nodes/router.py`
  传 `GraphStateSource(state)`、房间 `collab/room.py::add_constraint` 传
  `RoomSource(room)`，同经 `route_turn(context_source=...)` 进统一路由脑子）。
"""
