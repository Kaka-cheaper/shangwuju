...
def _plan_recap_clause(plan_version_log: tuple) -> str:
    ...
    材料来自会话上下文打包器的方案版本志切片（`RoutingContext.plan_version_
    log`）——narrate 排在 finalize_plan 之后（见 `agent/graph/build.py` 拓扑），
    本轮刚定稿的新版本条目此刻已在版本志末尾，其 `trigger` 字段（"feedback"/
    "first"，见 `finalize_plan._version_log_trigger`）就是"本版因何而生"的
    单一真相源——**仅 trigger=="feedback" 时产出回顾句**，首轮/全新解析返回
    空串（不硬扯，ADR 任务书原文纪律）。不重复读 `state.route_kind` 二次判定：
    版本志的 trigger 本就由它派生（finalize_plan 处），一个事实一处判。

    反馈原话从条目 summary（形如 "v2: 应『太远了』调整"）的『』引用里提取
    ——与 `finalize_plan._version_log_entry` 的写入格式对偶；格式对不上
    （未来 summary 措辞演化）时退化为不带引用的通用回顾句，不崩、不瞎编。
    """
    if not plan_version_log:
        return ""
    last = plan_version_log[-1]
    if last.get("trigger") != "feedback":
        return ""
    m = _RECAP_QUOTE_RE.search(str(last.get("summary") or ""))
    if m:
        return f"这版是照你『{m.group(1)}』的反馈调过的"
    return "这版是按你刚才的反馈调过的"
...
def narrate_node(state: AgentState) -> dict[str, Any]:
    ...
    # `_plan_recap_clause` docstring。不直接读 state["plan_version_log"]：
    # 上下文材料一律走打包器（ADR-0011"禁止各节点自己拼上下文"）。
