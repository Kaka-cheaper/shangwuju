...
# mock_data/pois.json 快照——没有词典/词汇表快捷词，但用户说这些词的原字面就能
# 被 poi_desire_match 直接 substring 命中——不是失配，是"词典没枚举全部类目"，
# 符合"不闭门造词"纪律不需要额外别名。
#
# 望京数据集切换批（2026-07-10）：顶层 mock_data/ 从"杭州+望京混存"改为
# "望京专属"（杭州全套已归档到 mock_data/hangzhou/，见该目录 conftest 隔离
# 拷贝源说明）。望京 POI type 汉化时已尽量对齐杭州既有 type 用词（电影院/
# 城市公园/密室/桌游馆/KTV/健身房/台球厅/书店/画廊/展览/livehouse/酒吧 等），
# 因此本集合比杭州快照小很多（杭州特有的 46 个 type 中 39 个孤儿，只剩望京
# 实际用到的这些）。曾有的"非餐饮"内部占位 type（798/UCCA 两条）已在返工批
# 修掉——它会经 narrate._poi_recommendation_reason 的「{type}，{首tag}」兜底
# 文案印上演示卡片（"非餐饮，看展"），不是纯内部字段；现 WJP001=展览（词汇
# 表词，非孤儿）、WJP002=街区漫步（既有孤儿词），孤儿集合相应缩小。
#
# 数据补齐批（2026-07-10 加码版）：新增 12 个真实望京 POI 引入 3 个新 type
# （商务茶室/茶馆/烘焙工坊——均为杭州集既有 type 用词，非造词）。三者都是
# 用户可直说的原字面（"找个茶馆"），substring 自命中，属正常新孤儿，照
# 快照协议加入集合。
#
# 终极批（2026-07-10 高德周边搜索加码）：新增 6 个孤儿 type——
# SPA/猫咖/剧本杀/瑜伽馆 为杭州集既有用词，轰趴馆/保龄球馆 为高德真实返回
# 的品类词（评委可直说的原字面）。全部 substring 自命中，照快照协议加入。
#
# 评论富化批事实纠错（2026-07-10 用户拍板"改"）：WJP056 北京卡拉河经真实
# 评价素材核实是宝可梦集换式卡牌店，不是桌游馆——type 桌游馆→卡牌店、
# tags 桌游→卡牌（评论已如实写"其实是卡牌店"，品类与评论不再自相矛盾）。
# "卡牌店/卡牌"为该店真实业态的原字面（"想打卡牌"substring 自命中），照
# 快照协议加入；桌游馆仍余 5 家，各品类地板不受影响。
_EXPECTED_ORPHAN_POI_TYPES = frozenset(
    {
        "livehouse", "书店", "亲子乐园", "健身房", "台球厅",
        "城市公园", "城市观光", "复合体验馆", "商务茶室", "电影院",
        "街区漫步", "茶馆", "酒吧", "烘焙工坊",
        "SPA", "猫咖", "剧本杀", "瑜伽馆", "轰趴馆", "保龄球馆",
        "卡牌店",
    }
)
