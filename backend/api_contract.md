...
> **协议变更（分界修缮批，91e0003）**：旧字段 `allowed_restaurant_ids` / `allowed_poi_ids` 已删除——全仓零消费，docstring 声称的白名单校验从未存在（虚假安全声明）。防编造的真实防线已换代为 **pending_actions 忠实回放**：`finalize_plan` 在规划期锁定确认动作清单，`replay_confirm_actions` 照单执行、执行期零决策，前端没有注入面。`ChatConfirmRequest` 为 `extra="forbid"`，继续发送这两个字段会得到 422。
...
1. memory_writer 副作用（行程档案 `recent_trips`——**按 session_id 键控**的进程内
   存储，6092223 身份读写分离批起运行时零文件写；`user_profile.json` 只作只读种子）
2. memory_store 标签/访问累积（`_accumulate_memory_after_confirm`，**按 session_id
   键控**；房间键=`collab_{room_id}`。"会话即身份"是 demo 无账号体系的显式设计，
   生产迁移=键换账号 ID，机制不动）
