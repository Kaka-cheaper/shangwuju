...
| modifications | 否 | `decision="modify"` 时的改动——**当前实现未消费此字段**（见 §10 发现的出入） |
...
## 10. 发现的文档-代码新出入（记录，不在本批修复范围）

- `ChatConfirmRequest.modifications` 字段仍存在于请求体 schema，但
  `_graph_confirm` 实现未读取/消费它——`decision="modify"` 分支与
  `decision="reject"` 走同一段代码：只推一条 `agent_thought` + `done`，
  不做任何"按 modifications 改后再确认"的逻辑。字段是死字段。
- `schemas/refine.py` 模块 docstring 仍写着"完整事件序列详见
  backend/api_contract.md §7"和"HTTP 端点（在 backend/main.py，B 块实现）"
  ——`/chat/refine` 端点已删除，该文件自身文档未跟着更新（不在本次三文件
  地盘内，未改；如需修，去改 `schemas/refine.py` 本身）。
- `CtaChip.action` 字段支持 `"confirm"`（一键触发确认、replay 已挂好的
  `pending_actions`）在 `schemas/router.py` 有描述，但后端 `api/chat.py` /
  `collab/room.py` 侧未见对应的"识别到 action=confirm 就短路直调
  `/chat/confirm`"逻辑——这个字段的消费方看起来完全在前端（前端如何处理
  超出后端契约文档范围，未深挖，仅记录"后端定义了但后端自己不消费"这一点）。
