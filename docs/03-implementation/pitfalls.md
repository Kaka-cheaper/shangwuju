...
### [P1] 2026-07-13 `docs/` 整体移出 Git，但后端镜像仍硬依赖 `docs/legal`

- **现象**：当前机器执行 `docker build -f backend/Dockerfile .` 能成功，是因为 `git rm --cached` 后本地仍保留并忽略了 `docs/legal/*.md`；全新 clone 不含这些文件，Dockerfile 的 `COPY docs/legal /app/docs/legal` 会直接失败，即使绕过 COPY，`/legal/terms` 与 `/legal/privacy` 也会返回 404。
- **根因**：仓库清理用 `/docs/` 的整体忽略规则移除了运行时资产，但 `backend/api/legal.py` 与 `backend/Dockerfile` 仍把两份法务 Markdown 当作生产依赖；“本地保留”掩盖了干净环境不可构建的问题。
- **本轮处理**：为不扩大部署范围，本次使用本机保留的法务文件完成镜像构建，并显式验证两个法务端点均为 200；未在部署过程中擅自改变仓库清理决策。
- **防再犯**：
  1. 运行时必需资产必须进入受版本控制的代码路径，例如 `backend/resources/legal/`，不能依赖被 `.gitignore` 排除的本地文件。
  2. CI 至少在干净 clone 中执行一次 `docker build --platform linux/amd64 -f backend/Dockerfile .`，防止本地残留文件掩盖缺失依赖。
  3. 做 `git rm --cached` 大清理前，先用 `rg` 检查 Dockerfile、代码读取路径和打包脚本是否引用待移除目录。
- **优先级**：P1（当前机器可部署，但换机器、CI 或灾难恢复时会直接无法构建生产镜像）。
