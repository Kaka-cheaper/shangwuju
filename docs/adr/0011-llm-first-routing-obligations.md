...
~~E-1 已知缺口:「重新规划一个」丢原始需求~~(**已修复,2026-07-03**:`FLOOR_REPLAN_SEND` 常量单一真相源;主聊天 intent 路径与房间 `_trigger_fresh_plan` 双消费点识别 canonical 字面即复用上一事件 raw_input 重解;时序前提=E-0-b 的重置发生在节点返回合并时而非进入时,旧 intent 替换判定时刻仍可读,测试连同该时序一并钉死;房间 5b 断言由"钉住退化"翻转为"真的重开新局")。
**E-2-a 已完成**(2026-07-03,c7638fd):messages 通道通水(router 写 HumanMessage——壳1拦截轮写占位、>500 字截断;chitchat 类与 narrate 叙事写 AIMessage;护栏 4/5 落注)+ 方案版本志 `plan_version_log`(SESSION_SCOPED,operator.add 纯追加归并器;常规写手 finalize_plan——trigger 只反映入口维度 first/feedback,主代理改判钉死"求解路径已住 decision_trace 不二存";confirm 一笔在 graph_confirm 同次 aupdate_state,闭合 E-0-b 记录的"下过单事实随重置丢失"窗口)。补笔(0fa0a2a):换菜路径(graph_adjust)同记版本志,trigger="adjust",三写手同款条目形状。
**E-2-b 已完成**(2026-07-03,c26213c):会话上下文打包器 `agent/context/`——RoutingContext 六件套+render_text() 确定性序列化;SessionContextSource 协议+GraphStateSource/RoomSource 双底座实现(决策 3"一个打包器多来源"落地,RoomSource 零改 room.py,房间侧版本志/画像诚实返空待后续 wiring);纯函数保险丝(40 轮/8K token,系数 1.5/字保守值,钉锚集——首轮/版本志/待澄清/台账/摘要/画像永不裁,钉锚超预算有定义降级);两消费切片先行接线——refiner 吃「版本志+台账生效条目」(闭"点击诉求全量重排不认账"窗口)、narration 吃版本志末条反馈回顾("这版是照你『××』的反馈调过的",仅 trigger=feedback,首轮/换菜/确认不硬扯)。pending_clarification 占位透传,E-3 落字段即自动通。
**E-2-c 已完成**(2026-07-03,5321d8d):统一路由脑子 `agent/routing/brain.py`+`brain_prompt.py`——一次 LLM 调用出 6 标签+槽位(node_ref/feedback_hint 松散提示,判定与抽取分离)+置信度;低置信度(<0.6)归并 clarify 且文案整体换壳3同款;失败哨兵走保守地板;confirm 强制钉死唯一「确认预约」chip(L0 禁令 1 工程化);八场景少样本("有点累了"双判例)。级联收口:classify_input+classify_dialogue_act 双 LLM 调用退役(ADR-0002 标 Superseded,字面确认/预约规则下放保留、BOOKING/CONFIRM 合流 confirm);QA/预约/确认/软约束四规则前移脑子前(读码证明旧行为=命中即无条件覆盖,零行为变化省调用);router_node 重复 detect_injection 收敛(RouteOutcome.injection_blocked);route_turn 增 context_source,房间接 RoomSource——决策 3 双底座两侧真实消费。RouteKind 7→6(图内 3 处 catch-all 零改动;InputKind 前后端契约同步)。A-D 测试迁移收官(test_router/test_router_context 整体退役,等价契约移入 test_routing_brain 25 例+图级 4 例+真 LLM 8 例全过,延迟 1.8-4.1s)。1363 passed 1 skipped;tsc 零错误。
**E-2 全部收口。**余 E-3(澄清链路,含 ADR-0014 挂钩两项)/E-4(轨道 A 语料+CI)。
