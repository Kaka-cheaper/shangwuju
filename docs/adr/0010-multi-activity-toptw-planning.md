...
- **软锚触发（定死，复用 `_DINING_FOCUSED_CONTEXTS`）**：饭被软锚 iff ① `social_context ∈ dining-focused 集`（商务/纪念日/家宴），或 ② 出行窗**结束点落在某饭点窗内或完整跨过该窗**且有 dietary 信号；否则涌现。**（②于 2026-07-03 放宽,用户拍板:原文"完整跨窗"在真 LLM 复测中被证边界取舍失当——14:00-19:00 的带娃局差 60 分钟不满足旧判据,晚饭被涌现放弃,违背常识;旧判据保留为新判据的子集。）**
  **（触发集 tristate 扩充，2026-07-11 四条不变式批 C5a 正式修订——本条原「规则定死不可扩大解释」文以此留痕）**：新增 `IntentExtraction.explicit_dining_requested` 三态显式信号，**双向压过上述全部推断触发**——`True`（明说要吃饭，含「找地方吃饭」类无品类泛化意愿）→ 恒软锚，「用户明说」本身就是充分条件；`False`（明说不要排饭，「吃过了/不用排饭」）→ 恒不锚，**抑制①②全部推断**（顺带修复既存缺陷：商务场景明说不用安排吃的仍被①硬塞商务餐）；`None`（没提及）→ 走①②原判据，行为分毫不变。这不是推翻「饭：条件性主角」——核心主张「看场景/诉求，不硬编码必到」完全保留，只是把「用户明说」补进「该排」判定；「明说不要」是同一宪法（显式压过推断）的反向应用。镜像纪律：`route_builder.dining_soft_anchored` 与 `blueprint_prompt.py`【饭：条件性主角】段是同一规则的两处独立实现，改动必须同步（本批已同步并配 prompt 镜像钉测试）。配套 critic 硬校验 `explicit_dining_missing`（True 且方案无餐厅节点 → HARD 驱动补饭；带窄池可行性护栏 + slot-hint 候选店名）。**连带**：blueprint prompt 长度 cap 2800→3000（本批两条 ADR 强制新规则——本条触发集 + ADR-0017 的 rationale 禁钟点纪律——原 cap 剩余空间仅 7 字符无处安放，沿「必要业务规则空间」既有扩容先例）。
...
**（advisory 家族扩员，2026-07-11 四条不变式批 C6 留痕）**：新增两码兑现 I4「缺席必须发声」——`MEAL_REQUESTED_UNSEATED`（显式要吃饭但试了没排上：「试了没成」+出路，**不是**「按设计缺席」）与 `MEAL_OMITTED_BY_DESIGN`（没提及、出行窗真实压过某饭点窗、按设计没排：「默认你们吃过来，想加跟我说」）。两码由 `agent.planning.critic.meal_absence.build_meal_absence_signal` **单一生成函数**、以 `explicit_dining_requested` 为**唯一分叉点**产出，结构上互斥（第三分叉 `False` 产无码轻确认句「按你说的，这次没排饭」）——杜绝「一边说按设计没排、一边实际是想排没排上」的口播自相矛盾。检测谓词 `meal_windows.crossed_meal_window`（完整区间重叠，45min 阈值，夜宵仅整窗落夜宵才点破）与决策 10 的软锚谓词是**两把尺子**（判「要不要强制排」有意收窄防误锚 vs 判「没排要不要说」必须完整防漏说），刻意不合并。
...
✅ **已全部落地**（决策 2026-07-01 · solve-it-right + grill-with-docs + 两轮全面红队复审 · 按 D-1→D-8 分步 test-first 实施，2026-07-02 收官；UX 验收 = S1-S8 结构差异 + 不静默）。
**D-6（S1-S8 结构验收 + shake 存废实测）已完成**：结论"实测砍除"（shake 不接线，见上方 D-6 条目的完整数据与判读）。
**D-7（pinning + advisory 通道）已完成**：`PinSpec`（schemas/pin.py，kind+target_id，时间钉留 intent 层立项时一并设计）+ `Advisory` 5 码（schemas/advisory.py）；`plan_hybrid` 接受 pinned、修复闭环 pin 默认保护/万不得已牺牲必记账；advisory 附着「最终交付方案」（成员资格过滤）、同码合并成单句防 narrator 截断自吞；链路 planner→state→narrate→AGENT_NARRATION.payload.messages（`{kind,code,text}`，按 ADR-0011 决策 5 统一消息面形状）；1073 passed 零回归。范围外遗留：intent 层 pin 抽取（单独立项）、rule 地板不产 advisory；~~主 LLM 蓝图路径的 SOFT 违规未上告知面~~（**已修复,2026-07-03**:narrate 把 `state.violations` 的 SOFT 转 advisory 并入 D-7 管道,按 message 精确去重防两路重复;顺带覆盖 rule 地板路径——critic→narrate 边两路共享的必然结果;决策 11 至此在全部三条规划路径兑现）。
**D-8（死代码清尾）已完成**：critic 部分（nodes_incomplete→「≥1 活动」）已由 D-8a 提前落地；本步删 5 段时代遗产——`decide_segments`/`explain_segments` 兼容别名、`FULL_SEGMENTS`/`ALWAYS_INCLUDED`/`FULL_MID_NODES` 常量、`segment_decider.py` 转发垫片（实测生产侧零调用方，其 docstring 声称的消费者列表早已失实）；rule 地板迁到 `decide_nodes` 正名（对下游行为逐位等价——全文件对段集合的成员查询只有主活动/用餐，过程段占位符从未被查询）；连带清 critic 层三处「按 decide_nodes→target_kind 判」镜像注释漂移与 rule_planner 模块标题改名遗留；`segment_decider` 进「必须 ImportError」回归清单；1070 passed（-4 恒真测试 +1 守卫，逐笔对平）。
