...
| MVP-3 演示 | 进行中   | 路演 V2 十页稿与可编辑 PPTX 已完成；剩替换 3 处素材占位 + 现场 dry run |
```
...
- **2026-07-13 路演 PPT V2**：沿用 `路演PPT/deck.html` 的暖纸张视觉语言，并吸收 `roadshow.html` 的卡片、细线时间轴与矩形媒体框组件，完成 10 页 `deck-v2.html` 与可编辑 `晌午局-路演-v2.pptx`。叙事收敛为「痛点 → Demo → 系统地图 → 对物（可排程、可执行、可返工）→ 对人（需求账可追溯）→ 测试证据 → 商业价值 → 二维码」。所有媒体保持矩形占位，待替换产品录屏、订单截图与线上二维码。验证：DOM 严格校验 0 问题；PowerPoint COM 成功打开并渲染 10/10 页；后端全量基线 1894 passed / 1 skipped / 1 xpassed。
...
- **D-FC-REDEPLOY-20260713** [2026-07-13]：当前 `main@bcfbb6bc7052` 已重新部署到阿里云 FC。
  - 镜像：`backend-20260713-bcfbb6b-121714`；ACR/FC 实际 Digest `sha256:0fcb56ca79aaec01b2e267e365b17a47c83c3a96eac01b3aa472308d3d61551f`。
  - 发布边界：仅更新 FC Custom Container 镜像，保留 0.5 vCPU / 1 GB / 120s、`instanceConcurrency=200`、Cookie 会话亲和、环境变量和 HTTP 触发器。
  - 验证：后端 `1894 passed, 1 skipped, 1 xpassed`；本地 AMD64 容器 `/health`、`/ready`、法务端点通过；公网真实 LLM 模式 `/ready=200`，创建房间 `ef594a` 并读取状态成功，CloudBase Origin CORS 预检通过。
