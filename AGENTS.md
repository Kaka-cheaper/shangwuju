...
### 3.3.1 编排层冻结纪律（2026-05-22 决议；2026-05-23 重组更新）

历史上 `backend/agent/` 有三套并存的编排实现（rule-based / Pydantic AI v2 / LangGraph graph），自 LangGraph 主架构上线 + spec `agent-directory-restructure` 重组后锁定为：
...
│
└── legacy/             ← 全部冻结，每个 .py 文件顶部含 # FROZEN: 详见 AGENTS.md §3.3.1
    ├── planner_rule.py        ← 重命名自 planner.py
    ├── ils_planner.py         ← 重命名自 planner_hybrid.py（LangGraph replan 第 3 次 ILS 兜底）
    ├── llm_first_planner.py   ← 重命名自 planner_llm_first.py
    ├── llm_planner.py         ← function_calling 旧实现
    ├── ils_score_critic.py    ← 重命名自 critics.py（旧 ILS 路径 critic）
    ├── executor.py            ← 与 graph/nodes/execute_finalize.py 双轨
    ├── segment_decider.py     ← 兼容 alias
    └── prompts/llm_planner_prompt.py
```
...
- 涉及 prompt 的改动按归属：意图层 prompt 在 `agent/intent/prompts/`；蓝图 prompt 在 `agent/planning/blueprint/prompts/`；冻结 prompt 在 `agent/legacy/prompts/`
- 加新文件前先确认归属（参考上面目录树）；不允许在 `agent/` 顶层加 `.py` 文件（除 `__init__.py`）

**MUST NOT**：

- 在 `agent/legacy/` 加新功能、新 Agent、新输出类型、新 critic 规则；只允许 bug fix + schema 适配
- 让 fallback 路径与主路径行为漂移（fallback 是 demo 兜底，不是 A/B 实验对照组）
- 删除 `agent/legacy/` 模块的 `# FROZEN` 标记（受 `verify_legacy_frozen.py` 守护）
- 删除冻结路径里的导出符号——多个 fallback 链路依赖它们（先 grep 确认零引用再删）
