...
### 3.3.1 编排层目录纪律（2026-05-22 决议；2026-05-23 重组更新；2026-05-24 spec D 修正——0 个真死代码）

历史上 `backend/agent/` 有三套并存的编排实现（rule-based / Pydantic AI v2 / LangGraph graph），自 LangGraph 主架构上线 + spec `agent-directory-restructure` 重组 + spec `legacy-cleanup-and-honest-naming` 修正后锁定为：

```text
agent/                  ← 6 子目录 + __init__（spec agent-directory-restructure 落地）
├── core/               ← 全员共享底座（llm_client / trace / observability_init / feedback_detector / llm_client_stub）
│
├── intent/             ← 意图理解 + 反馈刷新 + 文案输出
│   ├── parser.py       ← 重命名自 intent_parser.py
│   ├── refiner.py
│   ├── router.py
│   ├── narrator.py     ← 与 SOCIAL_CONTEXTS 词典强耦合，归 intent/
│   └── prompts/        ← intent_parser_prompt（重命名自 system_prompt）/ refiner_prompt / router_prompt / narrator_prompt
│
├── planning/           ← 规划主路径
│   ├── blueprint/
│   │   ├── blueprint.py          ← 含 _age_aware_duration_critic（spec planning-quality-deep-review R4）
│   │   ├── blueprint_llm.py      ← 含改后的 _poi_preview / _restaurant_preview（R2）
│   │   ├── assemble_blueprint.py
│   │   ├── node_decider.py       ← 仅决 kind，不决时长（adversarial §2 冲突 1 拒升级）
│   │   └── prompts/blueprint_prompt.py  ← 含 R3 改后的范例 + 分级表
│   ├── critic/
│   │   ├── critics_v2.py         ← 含 AGE_DURATION_MISMATCH 镜像（R4）
│   │   ├── social_compat.py
│   │   └── ils_score_critic.py   ← ILS 候选打分专用 critic（与 critics_v2 维度不同）
│   ├── planners/                 ← 主路径 + PLANNER_LLM_STRATEGY 三档子策略
│   │   ├── rule_planner.py             ← main.py 真链路核心 + 三档分发器（重命名自 planner_rule.py）
│   │   ├── ils_planner.py              ← PLANNER_LLM_STRATEGY=hybrid + graph replan 第 3 次 ILS 兜底
│   │   ├── llm_first_planner.py        ← PLANNER_LLM_STRATEGY=llm_first（**默认值**）核心生产路径
│   │   ├── llm_planner.py              ← PLANNER_LLM_STRATEGY=function_calling 子策略（A/B）
│   │   ├── segment_decider.py          ← ils_planner 依赖
│   │   └── prompts/llm_planner_prompt.py  ← llm_planner.py 的 system prompt
│   ├── execution/                ← 执行类活代码（与 graph/execute_finalize 行为不等价）
│   │   └── executor.py                 ← 用 _extract_reserved_time(note) 解析预留时段
│   ├── commute/lookup_hop.py
│   └── weights_llm.py            ← # FROZEN: 仅 ILS 路径
│
├── runtime/            ← Pydantic AI ReAct 运行时框架
│   ├── react_agent.py / output_types.py / orchestrator.py
│   ├── conversation.py / tool_provider.py / deps.py / model_factory.py / observability.py
│   └── tools/search_adapter.py
│
└── graph/              ← MAIN 路径（LangGraph Plan-and-Execute），不动 build.py 拓扑
    ├── nodes/ (11 个节点)
    ├── state.py / build.py / sse_adapter.py
```

**MUST**：

- 新功能必须在 `agent/graph/` 下加节点 / 改 state / 改 build.py
- 涉及 critic 的改动优先看 `agent/planning/critic/critics_v2.py` 是否需要双向兼容（被 graph/critic_node 共用）
- 涉及 Tool 调用的改动只动 `backend/tools/`，不要在编排层包装
...
- 不动 `graph/build.py` 拓扑——LangGraph 主路径是稳定 API
- 让 fallback 路径与主路径行为漂移（fallback 是 demo 兜底，不是 A/B 实验对照组）
