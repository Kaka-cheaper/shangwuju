...
### 3.3.1 编排层冻结纪律（2026-05-22 决议）

历史上 `backend/agent/` 有三套并存的编排实现，自 LangGraph 主架构上线后已锁定单一演进入口：

```text
agent/graph/        ← MAIN 路径（LangGraph Plan-and-Execute）
                      ✅ 所有新功能改动只在这里加
                      ✅ /chat/turn USE_LANGGRAPH=1（默认 ON）走这里

agent/v2/           ← FALLBACK 路径（Pydantic AI ReAct）
                      ⚠️ 冻结：USE_REACT_AGENT=1 fallback 链兜底用
                      ⚠️ 不要再加新 Agent / 新 critic 规则；critics_v2 被 graph 共用，可改但要双向兼容

agent/planner.py            ← SAFETY-NET（rule-based）
agent/planner_hybrid.py     ← LangGraph replan 第 3 次 ILS 兜底
agent/planner_llm_first.py  ← plan_itinerary_with_mode("llm") 子策略
agent/llm_planner.py        ← 同上（function_calling 旧实现）
                      ⚠️ 全部冻结：仅 bug fix + schema 适配；不加新功能
```

**MUST**：

- 新功能必须在 `agent/graph/` 下加节点 / 改 state / 改 build.py
- 涉及 critic 的改动优先看 `agent/v2/critics_v2.py` 是否需要双向兼容（被 graph/critic_node 共用）
- 涉及 Tool 调用的改动只动 `backend/tools/`，不要在编排层包装

**MUST NOT**：

- 在 `agent/v2/` / `agent/planner*.py` / `agent/llm_planner.py` 加新功能、新 Agent、新输出类型、新 critic 规则
